...
description: >
  ...
  translate the patch in a subagent, then run the revdiff skill.
when_to_use: >
...
4. Run the `revdiff` skill (`/revdiff:revdiff` under Claude Code) on the patch with `--stdin` — that is what renders a real diff with
