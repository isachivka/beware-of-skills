...
def describe_tail(pane: str, rows: int = 4, width: int = 90) -> str:
    """The last non-blank rows of the pane, for a refusal message to carry."""
    lines = [line.rstrip() for line in (pane or "").splitlines() if line.strip()][-rows:]
    if not lines:
        return "; the pane read back empty"
    return "; pane tail: " + " | ".join(line[:width] for line in lines)
...
@contextmanager
def pane_reachable(sid: str, profile: Profile, window: str | None = None):
    """Show a collapsed split for the duration of a send, then collapse it again.
    ...
    if not (hidden and profile.pane == "right"):
        yield False
        return
    ctl("session", "split", "visibility", "on", "--target", sid, *window_option(window))
    try:
        yield True
    finally:
        try:
...
def send(
    ...
    if prepared is None:
        ...
        typed = prepared
    with pane_reachable(sid, profile, window) as unhidden:
        if unhidden:
            time.sleep(COMPOSER_SETTLE_DELAY)   # let the surface exist before measuring
        return _send_visible(sid, profile, message, typed, window, delivery)
...
def _send_visible(
    ...
    try:
        ...
        if empty_text is None:
            # LOCAL PATCH (not upstream): quote what the parser saw. The bare reason lists
            # four causes and names none, and by the time anyone looks the screen has moved.
            raise PromptBlocked(
